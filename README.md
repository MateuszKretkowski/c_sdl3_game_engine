...
The engine currently runs peer-to-peer LAN multiplayer, a rigid body simulation with sphere-sphere and sphere-AABB collision response, textured meshes and a material system.

Two instances of a Pong scene on the same LAN, each owning its own paddle and replicating it to the other:

![LAN multiplayer](gifs/multiplayer_LAN.gif)

Rigid bodies, contact response and the debug renderer:
